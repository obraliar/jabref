...
public class OpenDatabaseAction extends MnemonicAwareAction {
    ...
    private BasePanel addNewDatabase(ParserResult result, final File file, boolean raisePanel) {
        ...
        BibDatabaseContext bibDatabaseContext = new BibDatabaseContext(database, meta, file, defaults);

        //TODO
        if (Objects.nonNull(database.getDatabaseID())) {
            // test
            DBMSConnectionProperties dbmsConnectionProperties = new DBMSConnectionProperties();
            dbmsConnectionProperties.setType(DBMSType.POSTGRESQL);
            dbmsConnectionProperties.setHost("localhost");
            dbmsConnectionProperties.setPort(DBMSType.POSTGRESQL.getDefaultPort());
            dbmsConnectionProperties.setDatabase("jabref2");
            dbmsConnectionProperties.setUser("postgres");

            try {

                BasePanel basePanel = new SharedDatabaseUIManager(frame).openNewSharedDatabaseTab(dbmsConnectionProperties);
                basePanel.getBibDatabaseContext().setDatabaseFile(file);
                basePanel.getBibDatabaseContext().getDatabase().setDatabaseID(database.getDatabaseID());

                if (Globals.prefs.getBoolean(JabRefPreferences.LOCAL_AUTO_SAVE)) {
                    // activate a new AutoSaver if enabled
                    new AutoSaver(basePanel.getBibDatabaseContext()).registerListener(new AutoSaveUIManager(basePanel));
                }
        ...
        BasePanel basePanel = new BasePanel(frame, bibDatabaseContext);
